...
var account_set_ops = {
    ...
    eventBind:function () {
        $(".wrap_account_set .save").click(function () {
            ...
            if (mobile.length < 2) {
                common_ops.tip("电话号码输入不规范！",mobile_target)
                return false
            }
            if (email.length < 2) {
                common_ops.tip("邮箱输入不规范！",email_target)
                return false
            }
            if (login_name.length < 2) {
                common_ops.tip("登录用户名输入不规范！",login_name_target)
                return false
            }
            if (login_pwd.length < 6) {
                common_ops.tip("密码输入不规范！",login_pwd_target)
                return false
            }
